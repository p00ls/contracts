...
contract P00lsStaking is AccessControl, Multicall {
    ...
    event VaultSetup(IERC20 indexed token, address indexed user, uint64 expiration);
    event Deposit(IERC20 indexed token, address indexed user, uint256 amount, uint256 extra, uint256 newWeight);
        ...
        onlyActiveVault(token, to)
    {
        ...
        if (extra > 0) {
            // cap extra to vault value
            extra = Math.min(extra, router.getAmountsOut(vault.value, _tokenToPools(token))[2]);
            // deposit tokens
        ...
        onlyExpiredVault(token, msg.sender)
    {
        ...
        uint256 value = vault.value + lock.splitter.release(msg.sender);
        uint256 extra = vault.extra;

        SafeERC20.safeTransferFrom(token, address(this), to, value);
        SafeERC20.safeTransferFrom(pools, address(this), to, extra);
    ...
    function _computeWeight(uint256 value, uint256 extra, uint256 factor) public view returns (uint256) {
        // TODO: consider extra
        // router.getAmountsOut(amount, _poolsToToken(token))[2]
