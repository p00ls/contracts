...
contract DutchAuctionManager is AccessControl {
    address public immutable template = address(new DutchAuction());
    ...
    event DutchAuctionCreated(address indexed token, address auction);
    ...
    function start(IERC20 token) external onlyRole(AUCTION_MANAGER_ROLE) returns (address) {
        ...
        emit DutchAuctionCreated(address(token), instance);

        // Send half of the token to the instance - keep the rest for the AMM
        SafeERC20.safeTransfer(token, instance, balance / 2);

        // Start auction
        DutchAuction(payable(instance)).initialize(
            token,
            block.timestamp, // startTime
            block.timestamp + 14 days, // endTime
            1 ether, // startPrice
            1 wei // minimumPrice
        );
    ...
    function finalize(IERC20 token) external onlyRole(AUCTION_MANAGER_ROLE) withPayments() {
        ...
        DutchAuction(payable(instance)).finalize();
